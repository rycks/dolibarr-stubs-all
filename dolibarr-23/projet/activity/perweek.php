...
$extrafieldsobjectkey = 'projet_task';
$tasksarray = $taskstatic->getTasksArray(\null, \null, $project->id ? $project->id : 0, $socid, 0, $search_project_ref, (string) $onlyopenedproject, $morewherefilter, $search_usertoprocessid ? $search_usertoprocessid : 0, 0, $extrafields, 0, $search_array_options_task, 1);
// We want to see all tasks of open project i am allowed to see and that match filter, not only my tasks. Later only mine will be editable later.
$tasksarraywithoutfilter = array();
$projectsrole = $taskstatic->getUserRolesForProjectsOrTasks($usertoprocess, \null, $project->id ? (string) $project->id : '0', 0, (int) $onlyopenedproject);
$tasksrole = $taskstatic->getUserRolesForProjectsOrTasks(\null, $usertoprocess, $project->id ? (string) $project->id : '0', 0, (int) $onlyopenedproject);
$param = '';
$search_array_options = $search_array_options_project;
$search_options_pattern = 'search_options_';
$search_array_options = $search_array_options_task;
